...
def finish_login():
    ...
    # Here we use Supabase's auth.users table. You could call a helper function
    # that upserts the user data via Supabase's REST API.
    createOrUpdateUser(
        user_info.get("name"),
        user_info.get("email"),
...
def createOrUpdateUser(name, email, pfp, access_token, refresh_token, expires_in):
    ...
    # (This example uses the Supabase client to call the REST API.)
    data = {
        "name": name,
        "email": email,
        "pfp": pfp,
    ...
    # Upsert means update if exists; insert otherwise.
...
def get_user_info_and_preferences(email, name):
    # Using Supabase client to fetch a user from "auth.users"
    response = supabase.table("auth.users").select("*").eq("email", email).eq("name", name).execute()
    user = response.data[0] if response.data else None
    # For preferences, assume you have a "user_preferences" table.
...
def fetch_drug_names():
    try:
        response = supabase.table("drugs").select("id, name, proper_name").execute()
        data = response.data
        if data:
...
def post_review():
    ...
    # For Supabase, insert into the "reviews" table.
    try:
...
def get_drug_reviews(drug_id):
    try:
        response = supabase.table("reviews").select("*").eq("target_type", "drug").eq("target_id", drug_id).order("created_at", desc=True).execute()
...
def get_vendor_reviews(vendor_id):
    try:
        response = supabase.table("reviews").select("*").eq("target_type", "vendor").eq("target_id", vendor_id).order("created_at", desc=True).execute()
        return jsonify({"status": "success", "reviews": response.data})
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500

# Example endpoint to log the request body (for debugging)
