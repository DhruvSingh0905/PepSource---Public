...
BATCH_FILE = "DB/Batch_requests/batch_input_summarization.jsonl"
OUTPUT_FILE = "DB/Batch_requests/batch_output_summarization.jsonl"
MODEL = "gpt-4o"
MAX_TOKENS = 1000
MAX_REQUESTS = 50000
...
def update_article_in_db(article_id: int, sections: dict):
    ...
    try:
        ...
        conn.commit()
        logger.info(f"Updated article ID {article_id} in local DB with AI summaries.")
    except Exception as e:
        logger.error(f"Error updating article ID {article_id} in local DB: {e}")
    finally:
        conn.close()

# --------------------------------------------------
# PROCESS BATCH RESULTS AND UPDATE LOCAL DB
# --------------------------------------------------
def process_batch_results():
    """
    Reads the batch results JSONL file, parses each line to extract the GPT response,
    and updates the corresponding article in the local database.
    ...
    logger.info(f"Finished processing batch results. Updated {processed_count} articles in local DB.")

# --------------------------------------------------
# UPLOAD UPDATED ARTICLES TO SUPABASE
# --------------------------------------------------
def upsert_ai_summaries_to_supabase():
    """
    Retrieves articles from the local DB that have non-empty AI summaries,
    ...
    cursor.execute("""
        ...
        WHERE ai_heading IS NOT NULL AND ai_heading <> ''
    """)
    articles = [dict(row) for row in cursor.fetchall()]
    conn.close()
    
    if not articles:
        logger.info("No articles with AI summaries to upsert to Supabase.")
        return
    
    # Upsert all articles (this will update existing rows)
    try:
        upsert_response = supabase.table("articles").upsert(articles, on_conflict="id").execute()
        logger.info(f"Upserted {len(articles)} articles with AI summaries to Supabase. Response: {upsert_response}")
    except Exception as e:
        logger.error(f"Error upserting articles to Supabase: {e}")

# --------------------------------------------------
# MAIN PROCESS: SEND, RECEIVE, PARSE, STORE, THEN UPLOAD TO SUPABASE
# --------------------------------------------------
if __name__ == "__main__":
    try:
        ...
        # Process the results and update the local database
        process_batch_results()
        
        # Upsert the updated articles (with AI summaries) to Supabase
        upsert_ai_summaries_to_supabase()
