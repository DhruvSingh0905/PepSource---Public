...
BATCH_FILE = "DB/Batch_requests/batch_input_side_effect_profiles.jsonl"
OUTPUT_FILE = "DB/Batch_requests/batch_output_side_effect_profiles.jsonl"
...
logger = logging.getLogger("side_effect_profiles_batch")
...
def ensure_side_effect_columns_exist():
    """
...
# --------------------------------------------------
def build_side_effect_prompt(drug_name, proper_name, what_it_does, how_it_works):
    """
    Constructs a prompt for OpenAI to provide side effect profiles for a drug.
    """
    prompt = f"""
...
def create_batch_requests():
    """
    Creates a JSONL batch file containing one request per drug for side effect profiles.
    """
    drugs = get_all_drugs()
    if not drugs:
    ...
    for drug in drugs:
        ...
        prompt = build_side_effect_prompt(name, proper_name, what_it_does, how_it_works)
        custom_id = f"drug{drug_id}_side_effects"
        
        logger.info(f"Creating batch request for drug ID {drug_id} side effect profile.")
        request_obj = {
            "custom_id": custom_id,
            "method": "POST",
...
def process_batch_results():
    """
    Reads the batch results JSONL file, parses each line to extract the side effect profiles,
    and updates the corresponding columns in the Drugs table.
    """
    ensure_side_effect_columns_exist()
    ...
    logger.info(f"Finished processing batch results. Updated side effect profiles for {processed_count} drugs.")
...
def upsert_drugs_to_supabase():
    """
    Retrieves drugs from the local DB that have been updated with side effect profiles,
    ...
    cursor.execute("""
        SELECT * FROM Drugs
        WHERE (side_effects_normal IS NOT NULL OR 
              side_effects_worrying IS NOT NULL OR 
              side_effects_stop_asap IS NOT NULL)
        AND in_supabase = 0
    """)
    drugs = [dict(row) for row in cursor.fetchall()]
    
    if not drugs:
        logger.info("No drugs with updated side effect profiles to upsert to Supabase.")
        return
    ...
    try:
        ...
        conn.commit()
        logger.info(f"Upserted {len(drugs)} drugs with side effect profiles to Supabase")
    except Exception as e:
