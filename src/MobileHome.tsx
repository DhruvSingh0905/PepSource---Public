  ...
  return (
    <div className="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-lg shadow-xl max-w-xs w-full mx-4">
        <h2 className="text-xl font-bold mb-4 text-gray-800 border-b border-gray-200 pb-3">
        ...
        >
          ...
          <div className="mt-6 flex justify-end">
            <button
              type="button"
              onClick={onClose}
              className="mr-2 px-4 py-2 border border-gray-300 text-gray-700 rounded-full text-sm hover:bg-gray-50 transition-colors"
            >
              Skip
            </button>
            <button
              type="submit"
              className="px-4 py-2 bg-[#3294b4] text-white rounded-full text-sm hover:bg-blue-600 transition-colors shadow-sm"
            >
              Submit
            </button>
          </div>
...
const DRUGS_PER_PAGE = 8; // Reduced for mobile
...
function MobileHome() {
  ...
  // Update drugs display when drugQueue changes
  useEffect(() => {
    setDrugsDisplayed(() => {
    ...
    // Initialize featured drugs - limit to fewer for mobile
    if (featuredDrugs.length === 0 && drugQueue.length > 0) {
      setFeaturedDrugs(drugQueue.slice(0, 10));
    }
  ...
  const handleCategoryChange = (category: string) => {
    ...
    // Hide dropdown after selection on mobile
